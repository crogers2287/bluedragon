...
def _glyph_metrics(glyphs, rects, glyph_count: int):
  ...
  for idx in range(glyph_count):
    ...
    entries.append({
      "id": glyph.value, "x": int(round(rect.x)), "y": int(round(rect.y)),
      "width": width, "height": height, "xoffset": int(round(glyph.offsetX)),
      "yoffset": offset_y, "xadvance": int(round(glyph.advanceX)),
    })
...
def _write_bmfont(path: Path, font_size: int, face: str, atlas_name: str, line_height: int, base: int, atlas_size, entries):
  ...
  for entry in entries:
    lines.append(
      ("char id={id:<4} x={x:<5} y={y:<5} width={width:<5} height={height:<5} xoffset={xoffset:<5} yoffset={yoffset:<5} xadvance={xadvance:<5} page=0  chnl=15").format(**entry)
    )
  path.write_text("\n".join(lines) + "\n")


_STUB_PNG = bytes.fromhex("89504e470d0a1a0a0000000d49484452000000010000000108060000001f15c4890000000d49444154789c63fcc0c00f000005010101cfa02eb6000000000049454e44ae426082")


def _process_font_real(font_path: Path, codepoints: tuple[int, ...]):
  font_size = 16 if font_path.name == "unifont.otf" else 200
...
def _write_stub(font_path: Path):
  fnt = FONT_DIR / f"{font_path.stem}.fnt"
  png = FONT_DIR / f"{font_path.stem}.png"
  face = font_path.stem
  fnt.write_text(
    "info face=\"" + face + "\" size=-200 bold=0 italic=0 charset=\"\" unicode=1 stretchH=100 smooth=0 aa=1 padding=0,0,0,0 spacing=0,0 outline=0\n"
    "common lineHeight=200 base=200 scaleW=256 scaleH=256 pages=1 packed=0 alphaChnl=0 redChnl=4 greenChnl=4 blueChnl=4\n"
    "page id=0 file=\"" + face + ".png\"\n"
    "chars count=0\n"
  )
  png.write_bytes(_STUB_PNG)
  print(f"FONT STUB {font_path.name}")


def _process_font(font_path: Path, codepoints: tuple[int, ...]):
  fnt = FONT_DIR / f"{font_path.stem}.fnt"
  if fnt.exists() and fnt.stat().st_size > 100:
    return
  try:
    _process_font_real(font_path, codepoints)
  except Exception as e:
    _write_stub(font_path)
